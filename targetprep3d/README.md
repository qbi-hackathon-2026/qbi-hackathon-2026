...
- **CLI:** `python -m targetprep3d.cli --protein EGFR`
- **MCP:** `python -m targetprep3d.mcp_server` (FastMCP server; one tool per pipeline stage)
...
```bash
uv sync --extra dev
uv run targetprep3d --protein EGFR
...
```bash
uv sync
uv run python -m targetprep3d.server    # then open http://127.0.0.1:8000
```
